...
class Problem:
    ...
    def _create_opportunity_json(self):
        return {'name': self.opportunity_name,
                'category': 'security',
                'cost_type': 'USD',
                'description': self.opportunity_description,
                'short_description': self.short_description,
                'severity': self.severity,
                'suggested_action_template': self.opportunity_suggested_action_template,
    ...
    def create_groovy_vuln_file(self):
        ...
        escaped_opportunity_action = self.information.replace('\n', '\\n')
        ...
        lines = ['package afip.vulns',
                 ...
                 '        setOpportunityName("{}")'.format(self.opportunity_name),
                 ...
                 '',
                 '    /**',
                 '     * If a variable is fed to one of the safety methods, it will be untainted.',
                 '     * These are the methods than sanitize or protect your variables from this vulnerablity',
                 '     */',
                 '    @SuppressWarnings( "GroovyUnusedDeclaration" )',
                 '    static ArrayList<String> getSafetyMethods() {',
                 '         return {}'.format(self.safety_methods or '[]'),
                 '    }',
                 '',
                 '}',
                 ]
    ...
    def _new_groovy_vuln_list(self, line, new_vuln):
        assert line.strip().startswith('private static final ArrayList<Class<Vuln>> vulnClasses = ')
        return self._change_line(line, new_vuln)

    def add_to_groovy_vulns(self):
        def is_correct_line(line):
            start = 'private static final ArrayList<Class<Vuln>> vulnClasses ='
    ...
    def add_to_groovy_detectors(self):
        ...
        def is_correct_line(line):
            return line.strip().startswith('private ArrayList<Class<Detector>> allDetectors = [')
